...
function loadingPublic(a) {
    $("#w").replaceWith(MPT.getTmpl("t_public_header", {cur: a || ""}));
    $("#f").replaceWith(MPT.getTmpl("t_public_footer"))
}
...
function countDown(d, b, a) {
    var c = 0;
    console.log(a == "reg" ? MPT.Config.req["getTimeReg"] : MPT.Config.req["getTime"]);
    $.ajax({
        type: "post",
