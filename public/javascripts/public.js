...
function loadingPublic(a) {
    $("#publicHeader").replaceWith(MPT.getTmpl("t_public_header", {cur: a || ""}));
    $("#publicFooter").replaceWith(MPT.getTmpl("t_public_footer"))
}
function loadingPublicTop(a) {
    $("#publicTop").replaceWith(MPT.getTmpl("t_public_top", a));
...
function loginSucc() {
    $.unblockUI();
    Utils.toTargetView(MPT.Config.url["space"]["passport"])
}
function countDown(d, b, a) {
    var c = 0;
    $.ajax({
        type: "post", url: (a == "reg" ? MPT.Config.req["getTimeReg"] : MPT.Config.req["getTime"]), data: "st=" + b + "&bt=" + a, dataType: "json", error: function () {
...
function genCapId(c, a) {
    ...
    var d = null;
    $.ajax({
        async: false, type: "get", url: MPT.Config.req["gencapid"], data: {ct: a}, success: function (f) {
            /*console.log(f)
